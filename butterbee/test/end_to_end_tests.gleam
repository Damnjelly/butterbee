...
pub fn table_row_test_() {
  ...
  let assert Ok(table_row) =
    driver
    |> test_page.data_table(table_element.Row(0), node.inner_text())
    |> driver.value()
...
pub fn table_cell_test_() {
  ...
  let assert Ok(cell_1_1) =
    driver
    |> test_page.data_table(table_element.Cell(0, 0), node.inner_text())
    |> driver.value()
  let assert Ok(cell_1_2) =
    driver
    |> test_page.data_table(table_element.Cell(0, 1), node.inner_text())
    |> driver.value()
  let assert Ok(cell_1_3) =
    driver
    |> test_page.data_table(table_element.Cell(0, 2), node.inner_text())
    |> driver.value()
  let assert Ok(cell_2_1) =
    driver
    |> test_page.data_table(table_element.Cell(1, 0), node.inner_text())
    |> driver.value()
  let assert Ok(cell_2_2) =
    driver
    |> test_page.data_table(table_element.Cell(1, 1), node.inner_text())
    |> driver.value()
  let assert Ok(cell_2_3) =
    driver
    |> test_page.data_table(table_element.Cell(1, 2), node.inner_text())
    |> driver.value()
  let assert Ok(cell_3_1) =
    driver
    |> test_page.data_table(table_element.Cell(2, 0), node.inner_text())
    |> driver.value()
  let assert Ok(cell_3_2) =
    driver
    |> test_page.data_table(table_element.Cell(2, 1), node.inner_text())
    |> driver.value()
...
pub fn list_item_test_() {
  ...
  let assert Ok(list_item) =
    driver
    |> test_page.test_list(list_element.Row(0), node.inner_text())
    |> driver.value()
  assert list_item == "First List Item"

  let assert Ok(list_item) =
    driver
    |> test_page.test_list(list_element.Row(1), node.inner_text())
    |> driver.value()
  assert list_item == "Second List Item"

  let assert Ok(list_item) =
    driver
    |> test_page.test_list(list_element.Row(2), node.inner_text())
    |> driver.value()
  assert list_item == "Third List Item"

  let assert Ok(list_item) =
    driver
    |> test_page.test_list(list_element.Row(3), node.inner_text())
    |> driver.value()
  assert list_item == "Fourth List Item"

  let assert Ok(list_item) =
    driver
    |> test_page.test_list(list_element.Row(4), node.inner_text())
    |> driver.close()
